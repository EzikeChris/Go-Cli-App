...
func (c *client) formatUserMsg(b []byte) (string, error) {
	...
	userS = cli.StringColorize(userS, userColor)
...
func formatSystemMsg(a any) string {
	switch msg := a.(type) {
	case string:
		return cli.StringColorize("System    : "+msg, cli.RedS)
	case []byte:
		return cli.StringColorize("System    : "+string(msg), cli.RedS)
	}
	return ""
}

func formatChatMsg(b []byte) string {
	return cli.StringColorize("Chat      : "+string(b), cli.Red)
}

func formatClientMsg(s string) string {
	return cli.StringColorize("You       : "+s, cli.WhiteS)
}
